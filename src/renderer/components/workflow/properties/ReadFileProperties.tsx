...
export default function ReadFileProperties({ node, updateNodeData }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1">编码格式</label>
        <select
          value={data.encoding}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor="errorIfNotFound" className="text-sm text-[var(--color-text)]">
          文件不存在时报错
        </label>
      </div>
